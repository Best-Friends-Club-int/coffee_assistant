/***********************
 * Coffee Akinator — final script.js
 * - Language selection wired via data-lang buttons
 * - Start screen texts per language
 * - Fun questions kept (no scoring)
 * - Core logic with method/drink constraints
 * - Referral + anti-cache on all outbound links
 ***********************/
...
const questions = [
  ...
  {
    text: {
      uk: "🍰 Улюблений десерт дитинства?",
      en: "🍰 Childhood favorite dessert?",
      es: "🍰 Postre favorito de la infancia?",
      ru: "🍰 Любимый десерт из детства?",
      pl: "🍰 Ulubiony deser z dzieciństwa?"
    },
    answers: [
  ...
  {
    text: {
      uk:"🍊 Який фрукт твій улюблений?",
      en:"🍊 Your favorite fruit?",
      es:"🍊 Tu fruta favorita?",
      ru:"🍊 Твой любимый фрукт?",
      pl:"🍊 Twój ulubiony owoc?"
    },
    answers: [
  ...
  {
    text:{
      uk:"🌸 Які парфуми тобі ближчі?",
      en:"🌸 Which perfumes are closer to you?",
      es:"🌸 ¿Qué perfumes te gustan más?",
      ru:"🌸 Какие духи тебе ближе?",
      pl:"🌸 Jakie perfumy są ci bliższe?"
    },
    answers:[
  ...
  {
    text:{
      uk:"☀️ Яка сцена тобі ближча?",
      en:"☀️ Which scene is closer to you?",
      es:"☀️ ¿Qué escena te gusta más?",
      ru:"☀️ Какая сцена ближе тебе?",
      pl:"☀️ Jaka scena jest ci bliższa?"
    },
    answers:[
  ...
  {
    text:{
      uk:"🍸 Який коктейль твій улюблений?",
      en:"🍸 Your favorite cocktail?",
      es:"🍸 Tu cóctel favorito?",
      ru:"🍸 Твой любимый коктейль?",
      pl:"🍸 Twój ulubiony koktajl?"
    },
    answers:[
  ...
  {
    text:{
      uk:"🌿 Як ти любиш проводити вихідні?",
      en:"🌿 How do you prefer weekends?",
      es:"🌿 ¿Cómo prefieres pasar los fines de semana?",
      ru:"🌿 Как ты любишь проводить выходные?",
      pl:"🌿 Jak wolisz spędzać weekendy?"
    },
    answers:[
  ...
  {
    text:{
      uk:"🫖 Який метод заварювання тобі ближче?",
      en:"🫖 Which brew method do you prefer?",
      es:"🫖 ¿Qué método de preparación prefieres?",
      ru:"🫖 Какой метод заваривания тебе ближе?",
      pl:"🫖 Jaki sposób parzenia wolisz?"
    },
    answers:[
  ...
  {
    text:{
      uk:"☕ Який кавовий напій тобі ближче?",
      en:"☕ Which coffee drink do you prefer?",
      es:"☕ ¿Qué bebida prefieres?",
      ru:"☕ Какой кофейный напиток тебе ближе?",
      pl:"☕ Jaki napój kawowy wolisz?"
    },
    answers:[
...
function tAnswer(a) {
  // returns localized answer text with fallback to EN
  return a.text[userLang] || a.text.en;
}

function tQ(q) {
  return q.text[userLang] || q.text.en;
}
...
function buildLink(base) {
  // Referral + anti-cache + UTMs to beat caching on destination side
  const ref = encodeURIComponent(getRef());
  const ts  = Date.now();
  const url = new URL(base);
  // keep existing query if any, then add our params
  url.searchParams.set("ref", ref);
  url.searchParams.set("utm_source", "akinator");
  url.searchParams.set("utm_medium", "quiz");
  url.searchParams.set("utm_campaign", "coffee_match");
  url.searchParams.set("t", ts.toString());
  return url.toString();
}

/* ---------------- Language selection ---------------- */
function selectLanguage(lang) {
  userLang = ["uk","en","es","ru","pl"].includes(lang) ? lang : "en";

  // Update start screen texts
  const pack = startTranslations[userLang] || startTranslations.en;
  if (mainTitleEl) mainTitleEl.textContent = pack.mainTitle;
  startScreen.querySelector("h2").textContent = pack.title;
  startScreen.querySelector("p").textContent  = pack.text;
  startBtn.textContent = pack.button;

  // Show start, hide lang
  langScreen.classList.add("hidden");
  startScreen.classList.remove("hidden");

  // Persist
  try { localStorage.setItem("akinator_lang", userLang); } catch {}
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Autoselect from storage (optional QoL): if user picked before, restore
  try {
    const saved = localStorage.getItem("akinator_lang");
    if (saved && ["uk","en","es","ru","pl"].includes(saved)) {
...
function showQuestion() {
  // If user chose filter method and current is drink question -> skip to result
  const q = questions[currentQ];
  if (selectedMethod === "filter" && q.answers.some(a => a.drink)) {
    showResult();
    return;
  }

  quizEl.innerHTML = `<h2>${tQ(q)}</h2>`;
  ...
  q.answers.forEach(a => {
    ...
    // anti-cache for images
    const imgSrc = `${a.img}?t=${Date.now()}`;
    card.innerHTML = `<img src="${imgSrc}" alt=""><p>${tAnswer(a)}</p>`;
...
function showResult() {
  let pool = [...coffeeProfiles];

  // HARD constraint for filter: only filter category, show best as main, second as alt
  if (selectedMethod === "filter") {
    pool = pool.filter(c => c.category === "filter");
    // score within filter by userProfile to pick main
    const scored = pool
      .map(c => {
        let s = 0;
    ...
    const main = scored[0] || pool[0];
    const alt  = scored[1] || pool[1];

    renderResult(main, alt ? [alt] : []);
    return;
  }

  // For milk/cappuccino, hide filter coffees entirely
  if (selectedDrink === "milk" || selectedDrink === "cappuccino") {
    pool = pool.filter(c => c.category !== "filter");
  }

  // For straight espresso, keep filters only 10% of the time
  ...
  // Score by overlap of userProfile tags with coffee tags
  const scored = pool
    .map(c => {
      let s = 0;
      for (const [tag, w] of Object.entries(userProfile)) {
      ...
      // Light bonus if coffee supports selected drink/method (when present)
      if (selectedDrink && c.tags[selectedDrink]) s += 0.5;
      if (selectedMethod && c.tags[selectedMethod]) s += 0.5;
      return { ...c, score: s };
    })
    .sort((a,b) => b.score - a.score);
  ...
  renderResult(main, recs);
}

function renderResult(mainCoffee, recommendations) {
  const phrase = rand(endPhrases[userLang] || endPhrases.en);

  let html = `
    <h2>${mainCoffee.name}</h2>
    <img src="${mainCoffee.img}?t=${Date.now()}" alt="${mainCoffee.name}">
    <div class="final-phrase">${phrase}</div>
    <a href="${buildLink(mainCoffee.link)}" target="_blank">
      <button>☕ ${userLang === "uk" ? "Замовити" :
                   userLang === "es" ? "Pedir" :
                   userLang === "pl" ? "Zamów" :
                   userLang === "ru" ? "Заказать" : "Order"}</button>
    </a>
  `;

  if (recommendations && recommendations.length > 0) {
    html += `<h3>✨ ${
      userLang === "uk" ? "Вам також може сподобатися:" :
      userLang === "es" ? "También te puede gustar:" :
      userLang === "pl" ? "Może Ci się też spodobać:" :
      userLang === "ru" ? "Вам также может понравиться:" :
      "You may also like:"
    }</h3><div class="gallery">`;

    recommendations.forEach(c => {
      html += `
        <a href="${buildLink(c.link)}" target="_blank" class="gallery-item" style="text-decoration:none;">
